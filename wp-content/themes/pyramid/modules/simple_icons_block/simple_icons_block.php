...
<?php if (have_rows('s_columns')) {
	wp_enqueue_style('simple_icons_block_styles', get_template_directory_uri() . '/static/css/modules/simple_icons_block/simple_icons_block.css', [], null);
	...
	<section class="simple_icons_block
        <?php
				...
				if (have_rows('s_columns')) { ?>
					<div class="columns wow fadeIn">
						<?php while (have_rows('s_columns')) : the_row();
							...
							<div class="item">
								<div class="icon">
									<?php echo wp_get_attachment_image($icon, 's_140'); ?>
								</div>
