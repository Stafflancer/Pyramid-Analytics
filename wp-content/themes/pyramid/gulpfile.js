 ...
 */
const baseDir = 'http://localhost:8888/pyramid';
...
function scripts() {
	...
	.pipe(concat('main.min.js'))	// Concat in one file main.min.js.
	.pipe(uglify())					// Process.
...
function styles() {
	...
	.pipe(autoprefixer({overrideBrowserslist: ['last 2 versions']}))	// Add prefixes.
...
// Minify images.
function images() {
	return src('src/images/**/*')	// Get all files from app/img/src/ directory.
	.pipe(newer('static/images'))	// Only new images (not in destination directory).
	.pipe(imagemin())				// Minimize images.
	.pipe(dest('static/images/'))	// Output.
}
